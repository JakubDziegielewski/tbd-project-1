locals {
  ...
  dbt_git_repo            = "https://github.com/JakubDziegielewski/tbd-tpc-di.git"
...
module "vertex_ai_workbench" {
  ...
  machine_type = var.machine_type_jupyter
...
module "dataproc" {
  ...
  number_of_preemptible_machines = var.number_of_preemptible_machines
}
